    ...
    public function save_candidature_his (Request $request)
    {
        $request->validate([
         ...
         // Création de la demande d'admission associée
            ...
            $demandeAdmission->etat = 'EN ATTENTE'; // Assigner la valeur 'EN ATTENTE' à la colonne 'etat'
            $demandeAdmission->decision = 'EN ATTENTE'; // Assigner la valeur 'EN ATTENTE' à la colonne 'etat'


            $demandeAdmission->save();


            return redirect()->route('saveEspaceCandidature',['id' => $candidat->idCand])->with('status','Félicitations ! Votre candidature a été validée');
